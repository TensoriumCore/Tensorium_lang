...
struct LowerContractToEinsum final
  ...
  LogicalResult matchAndRewrite(tensorium::mlir::DtAssignOp op,
    ...
    auto lowerProduct = [&](Value expr) -> FailureOr<Value> {
      ...
      auto eins =
          rewriter.create<tensorium::mlir::EinsumOp>(loc, resultType, inputs);
      eins->setAttr("spec", rewriter.getStringAttr(spec));
...
struct LowerContractOp final : OpRewritePattern<tensorium::mlir::ContractOp> {
  ...
  LogicalResult matchAndRewrite(tensorium::mlir::ContractOp op,
    ...
    auto eins = rewriter.create<tensorium::mlir::EinsumOp>(op.getLoc(),
                                                           resultType, inputs);
    eins->setAttr("spec", rewriter.getStringAttr(spec));
