...
RUNNER_SRC="$ROOT_DIR/tools/dev/runtime_generated_two_puncture_solve.cpp"
...
LL_PATH="/tmp/tensorium_generated_two_puncture_solve.ll"
OBJ_PATH="/tmp/tensorium_generated_two_puncture_solve.o"
HOST_HEADER="/tmp/tensorium_generated_two_puncture_solve_host.h"
EXE_PATH="/tmp/tensorium_generated_two_puncture_solve_runner"
...
echo "[two-puncture] generating LLVM IR and host header"
"$DRIVER" \
...
if ! grep -q "define void @tensorium_spectral_residual_grid_H" "$LL_PATH"; then
  echo "error: expected generated spectral residual grid kernel" >&2
  exit 2
fi

echo "[two-puncture] compiling generated LLVM and runtime runner"
...
echo "[two-puncture] running mapped manufactured solve"
"$EXE_PATH"
