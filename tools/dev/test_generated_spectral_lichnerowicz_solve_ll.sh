...
RUNNER_SRC="$ROOT_DIR/tools/dev/runtime_generated_spectral_lichnerowicz_solve.cpp"
...
LL_PATH="/tmp/tensorium_generated_spectral_lichnerowicz_solve.ll"
OBJ_PATH="/tmp/tensorium_generated_spectral_lichnerowicz_solve.o"
HOST_HEADER="/tmp/tensorium_generated_spectral_lichnerowicz_solve_host.h"
EXE_PATH="/tmp/tensorium_generated_spectral_lichnerowicz_solve_runner"
...
echo "[generated-spectral-lichnerowicz] generating LLVM IR and host header"
"$DRIVER" \
...
if ! grep -q "tensorium_spectral_residual_H" "$HOST_HEADER"; then
  echo "error: expected Lichnerowicz spectral residual symbol" >&2
  exit 2
fi
if ! grep -q "tensorium_spectral_residual_grid_H" "$HOST_HEADER"; then
  echo "error: expected Lichnerowicz spectral grid residual symbol" >&2
  exit 2
fi
...
echo "[generated-spectral-lichnerowicz] compiling LLVM object"
...
echo "[generated-spectral-lichnerowicz] compiling runtime runner"
"$CXX_BIN" -O0 -std=c++20 -I "$ROOT_DIR/include" -include "$HOST_HEADER" \
  "$RUNNER_SRC" "$OBJ_PATH" -lm -o "$EXE_PATH"

echo "[generated-spectral-lichnerowicz] running runtime executable"
"$EXE_PATH"
