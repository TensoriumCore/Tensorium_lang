...
int main(void) {
  ...
  printf("[ll-smoke] Reissner-Nordstrom Christoffel center point M=%.17g Q=%.17g "
         "r=%.17g theta=%.17g\n",
         M, Q, r[cidx], theta[cidx]);
  ...
  const double f = 1.0 - 2.0 * M / r0 + (Q * Q) / (r0 * r0);
  const double fp = (2.0 * M) / (r0 * r0) - (2.0 * Q * Q) / (r0 * r0 * r0);
  const double expected_r_rr = -0.5 * fp / f;
  ...
  if (!ok) {
    fprintf(stderr, "Reissner-Nordstrom Christoffel mismatch beyond tolerance\n");
    return 3;
  }
  return 0;
}
