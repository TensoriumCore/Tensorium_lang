...
RUNNER_SRC="$ROOT_DIR/tools/dev/ll_rhs_runner_schwarzschild_christoffel.c"
...
LL_PATH="/tmp/tensorium_schw_christoffel.ll"
OBJ_PATH="/tmp/tensorium_schw_christoffel_ll.o"
EXE_PATH="/tmp/tensorium_schw_christoffel_ll_runner"
GENERATE_LL=1
...
Options:
  --ll <path>      Path to LLVM IR file (default: /tmp/tensorium_schw_christoffel.ll)
...
if [[ $GENERATE_LL -eq 1 ]]; then
  echo "[ll-smoke] generating Christoffel LLVM IR: $LL_PATH"
  RAW_OUT="$(mktemp)"
  "$DRIVER" \
...
echo "[ll-smoke] compiling Christoffel LLVM IR object"
...
echo "[ll-smoke] compiling Christoffel C runner + linking"
"$CC_BIN" -O0 -std=c11 "$RUNNER_SRC" "$OBJ_PATH" -lm -o "$EXE_PATH"

echo "[ll-smoke] running Christoffel executable"
"$EXE_PATH"
