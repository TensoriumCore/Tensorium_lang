...
RUNNER_SRC="$ROOT_DIR/tools/dev/ll_rhs_runner_bssn_kasner_full.c"
...
LL_PATH="/tmp/tensorium_bssn_kasner_full.ll"
OBJ_PATH="/tmp/tensorium_bssn_kasner_full.o"
EXE_PATH="/tmp/tensorium_bssn_kasner_full_runner"
HOST_HEADER="/tmp/tensorium_bssn_kasner_full_host.h"
...
echo "[analytic-bssn] generating LLVM IR and host header: $LL_PATH"
"$DRIVER" \
...
echo "[analytic-bssn] compiling LLVM object"
...
echo "[analytic-bssn] compiling complete BSSN analytic runner"
"$CC_BIN" -O0 -std=c11 -include "$HOST_HEADER" "$RUNNER_SRC" "$OBJ_PATH" \
  -lm -o "$EXE_PATH"

echo "[analytic-bssn] running complete BSSN analytic executable"
"$EXE_PATH"
