...
RUNNER_SRC="$ROOT_DIR/tools/dev/ll_rhs_runner_bssn_reduced_rk2.c"
...
LL_PATH="/tmp/tensorium_bssn_reduced_rk2.ll"
OBJ_PATH="/tmp/tensorium_bssn_reduced_rk2.o"
EXE_PATH="/tmp/tensorium_bssn_reduced_rk2_runner"
...
echo "[ll-smoke] generating LLVM IR (bssn reduced rk2): $LL_PATH"
RAW_OUT="$(mktemp)"
...
echo "[ll-smoke] compiling LLVM IR object (bssn reduced rk2)"
...
echo "[ll-smoke] compiling BSSN reduced RK2 C runner + linking"
"$CC_BIN" -O0 -std=c11 "$RUNNER_SRC" "$OBJ_PATH" -lm -o "$EXE_PATH"

echo "[ll-smoke] running BSSN reduced RK2 executable"
"$EXE_PATH"
