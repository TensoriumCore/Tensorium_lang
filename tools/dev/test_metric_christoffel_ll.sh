...
LL_PATH="/tmp/tensorium_${CASE_NAME}_christoffel.ll"
OBJ_PATH="/tmp/tensorium_${CASE_NAME}_christoffel_ll.o"
EXE_PATH="/tmp/tensorium_${CASE_NAME}_christoffel_ll_runner"

echo "[ll-smoke] generating Christoffel LLVM IR ($CASE_NAME): $LL_PATH"
RAW_OUT="$(mktemp)"
"$DRIVER" \
...
echo "[ll-smoke] compiling Christoffel LLVM IR object ($CASE_NAME)"
...
echo "[ll-smoke] compiling Christoffel C runner + linking ($CASE_NAME)"
"$CC_BIN" -O0 -std=c11 "$RUNNER_SRC" "$OBJ_PATH" -lm -o "$EXE_PATH"

echo "[ll-smoke] running Christoffel executable ($CASE_NAME)"
"$EXE_PATH"
