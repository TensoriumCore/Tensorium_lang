...
FIXTURE="$ROOT_DIR/tests/fixtures/elliptic/spectral_hamiltonian_toy_nonlinear_3d.tn"
RUNNER_SRC="$ROOT_DIR/tools/dev/runtime_generated_spectral_nonlinear_solve.cpp"
...
LL_PATH="/tmp/tensorium_generated_spectral_nonlinear_solve.ll"
OBJ_PATH="/tmp/tensorium_generated_spectral_nonlinear_solve.o"
HOST_HEADER="/tmp/tensorium_generated_spectral_nonlinear_solve_host.h"
EXE_PATH="/tmp/tensorium_generated_spectral_nonlinear_solve_runner"
...
echo "[generated-spectral-nonlinear] generating LLVM IR and host header"
"$DRIVER" \
...
if ! grep -q "tensorium_spectral_residual_H" "$HOST_HEADER"; then
  echo "error: expected nonlinear spectral residual symbol" >&2
  exit 2
fi
if ! grep -q "tensorium_spectral_residual_grid_H" "$HOST_HEADER"; then
  echo "error: expected nonlinear spectral grid residual symbol" >&2
  exit 2
fi
...
if ! grep -q "SpectralHamiltonianToyNonlinear3D" "$HOST_HEADER"; then
  echo "error: expected nonlinear spectral system name" >&2
  exit 2
fi
if ! grep -q "define void @tensorium_spectral_residual_grid_H" "$LL_PATH"; then
  echo "error: expected nonlinear spectral grid residual LLVM definition" >&2
  exit 2
fi

echo "[generated-spectral-nonlinear] compiling LLVM object"
...
echo "[generated-spectral-nonlinear] compiling runtime runner"
"$CXX_BIN" -O0 -std=c++20 -I "$ROOT_DIR/include" -include "$HOST_HEADER" \
  "$RUNNER_SRC" "$OBJ_PATH" -lm -o "$EXE_PATH"

echo "[generated-spectral-nonlinear] running runtime executable"
"$EXE_PATH"
