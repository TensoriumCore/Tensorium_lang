...
double bump1DDerivative(double value) {
...
double bumpSecondDerivative(int axis, double x, double y, double z) {
  const double bx = bump1D(x);
  const double by = bump1D(y);
  const double bz = bump1D(z);
  if (axis == 0)
    return bump1DLaplacian(x) * by * bz;
  if (axis == 1)
    return bx * bump1DLaplacian(y) * bz;
  return bx * by * bump1DLaplacian(z);
}

double bumpMixedSecondDerivative(int axisA, int axisB, double x, double y,
                                 double z) {
  if (axisA == axisB)
    return bumpSecondDerivative(axisA, x, y, z);
  const double values[] = {x, y, z};
  double out = 1.0;
  for (int axis = 0; axis < 3; ++axis) {
    if (axis == axisA || axis == axisB)
      out *= bump1DDerivative(values[axis]);
    else
      out *= bump1D(values[axis]);
  }
  return out;
}
...
double exactW(int component, double x, double y, double z) {
  const double amplitudes[] = {0.018, -0.014, 0.011};
  return amplitudes[component] * bump(x, y, z);
}

double lapExactW(int component, double x, double y, double z) {
  const double amplitudes[] = {0.018, -0.014, 0.011};
  return amplitudes[component] * bumpLaplacian(x, y, z);
}

double yorkExactW(int component, double x, double y, double z) {
  const double amplitudes[] = {0.018, -0.014, 0.011};
  double divGradient = 0.0;
  for (int sourceComponent = 0; sourceComponent < 3; ++sourceComponent) {
    divGradient += amplitudes[sourceComponent] *
                   bumpMixedSecondDerivative(component, sourceComponent, x, y,
                                             z);
  }
  return lapExactW(component, x, y, z) + divGradient / 3.0;
}
  ...
  try {
      ...
      for (std::size_t j = 0; j < grid.n2(); ++j) {
        ...
        for (std::size_t i = 0; i < grid.n1(); ++i) {
          ...
          for (int component = 0; component < 3; ++component)
            exactFields[component + 1][p] = exactW(component, x, y, z);
          a2[p] = freeA2(x, y, z);
          ...
          for (int component = 0; component < 3; ++component) {
            jFields[component][p] =
                -(yorkExactW(component, x, y, z) +
                  vectorMass * exactFields[component + 1][p]);
          }
    ...
    const auto exactResidual = assembleSpectralResidualSystem(
        ...
        std::span<const std::vector<double>>(exactFields.data(),
                                             exactFields.size()));
    std::printf("[generated-spectral-york-vector] exact residual l2 = %.17g max = %.17g\n",
    ...
    if (!exactResidual.finite || !exactResidual.usedGeneratedGridKernels ||
      ...
      std::fprintf(stderr, "York vector exact residual mismatch\n");
      return 3;
    }
    ...
    std::printf("[generated-spectral-york-vector] steps = %d status = %d\n",
                solveResult.steps, static_cast<int>(solveResult.status));
    std::printf("[generated-spectral-york-vector] final residual l2 = %.17g\n",
                solveResult.finalResidualL2);
    std::printf("[generated-spectral-york-vector] final residual max = %.17g\n",
                finalResidual.maxAbs);
    std::printf("[generated-spectral-york-vector] solution max error = %.17g\n",
                solutionError);
    ...
    if (!run.initialResidual.usedGeneratedGridKernels ||
        ...
        solutionError > 2e-7) {
      std::fprintf(stderr, "generated York vector spectral solve mismatch\n");
      return 4;
    }
  } catch (const std::exception &ex) {
    std::fprintf(stderr,
                 "generated York vector spectral runner failed: %s\n",
                 ex.what());
    return 2;
  }
  return 0;
