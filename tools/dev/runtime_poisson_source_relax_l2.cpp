...
RuntimeViews bindViews(GeneratedHostStorage &storage) {
  ...
  views.source = storage.data("field:source");
...
void initializeSourceProblem(RuntimeViews views, std::int64_t nx,
                             std::int64_t ny, std::int64_t nz) {
  constexpr double pi = 3.14159265358979323846264338327950288;
  ...
  for (std::int64_t i = 0; i < nx; ++i) {
    for (std::int64_t j = 0; j < ny; ++j) {
      for (std::int64_t k = 0; k < nz; ++k) {
        ...
        const double exact = sineMode(i, j, k, nx, ny, nz);
        views.u[p] = 0.0;
        views.v[p] = 0.0;
        views.source[p] = lambdaDiscrete * exact;
        views.H[p] = 0.0;
      }
    }
  }
...
int main() {
  ...
  try {
    ...
    std::fprintf(stderr, "poisson source relaxation runner failed: %s\n",
                 ex.what());
    return 2;
  }
