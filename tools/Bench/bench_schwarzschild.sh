...
for f in "${FIXTURES[@]}"; do
  ...
  log="$OUT/${base}.log"
  {
    ...
    echo "-- mlir codegen --"
    /usr/bin/time -p "$BIN" --mlir-pass-timing --tensorium-einstein-lower --tensorium-index-analyze --tensorium-einstein-analyze-einsum --tensorium-einstein-canonicalize --tensorium-einstein-validate --dump-mlir "$f" >/dev/null
  } 2>&1 | tee "$log"
  echo "wrote $log"
done
